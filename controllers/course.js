...
const main = async(ctx, next) => {
    try {
        let dbcourse = await Usermodel.getCourse(ctx.state.token.ZJUid);
        if (dbcourse.length && dbcourse[0].course) {
            // console.log(dbcourse[0].course);
            // console.log(233);
            ctx.response.body = dbcourse[0].course;
            ...
            await Usermodel.saveCourse(ctx.state.token.ZJUid, JSON.stringify(courses));
