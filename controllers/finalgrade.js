...
const step_1 = (cookie) => {
    return new Promise((resolve, reject) => {
        var req = unirest("GET", "http://scrsprd.zju.edu.cn/psc/CSPRD_1/EMPLOYEE/HRMS/c/SSR_STUDENT_ACAD_REC_FL.SS" +
                "R_ACAD_REC_FL.GBL");
...
const step_2 = (cookie, lastNum, ics) => {
    return new Promise((resolve, reject) => {
        ...
        var req = unirest("POST", "http://scrsprd.zju.edu.cn/psc/CSPRD_1/EMPLOYEE/HRMS/c/SSR_STUDENT_ACAD_REC_FL.SS" +
                "R_ACAD_REC_FL.GBL");
...
const get_semester = async(ctx, next) => {
    try {
        let data_pp = {
        ...
        let {ICStateNum, ics} = await step_1(cookie);
        let {newNum, semester} = await step_2(cookie, ICStateNum, ics);
        ctx.response.body = semester;
        // console.log(semester);
    } catch (error) {
...
const get_grade = async(ctx, next) => {
    try {
        let data_pp = {
            "userid": ctx.state.token.ZJUid,
            "pwd": ctx.state.token.password_bb,
        ...
        let GPA = await step_3(cookie, newNum, ics, term);
        ctx.response.body = GPA;
    } catch (error) {
        ctx.response.body = error;
    }
};
...
module.exports = [
    {
        method: 'get',
        path: '/semester',
        controller: get_semester,
        auth: true
    }, {
        method: 'get',
        path: '/semester/:term',
        controller: get_grade,
        auth: true
    }
];